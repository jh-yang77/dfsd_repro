...
from dfsd_repro.models.route import *
    ...
    def _calc_integrad_scores(self, inputs, labels, iterations):
        ...
        for module in self.modules():
            if isinstance(module, nn.AvgPool2d):
                ...
                self.integrad_scores[len(self.integrad_scores) - 1] = self.integrad_scores[
                                                                          len(self.integrad_scores) - 1] / (
                                                                                  iterations + 1) * \
                                                                      original_activations[i]
    ...
    def _hook_layers(self):
        ...
        def forward_hook_relu(module, input, output):
            ...
            if self.pruned_activations_mask:
                output = torch.mul(output, self.pruned_activations_mask[len(self.activations)].to(
                    self.device))  # + self.pruning_biases[len(self.activations)].to(self.device)
