...
def compute_traditional_ood(base_dir, in_dataset, out_datasets, method, name,p,evr):
    # print('Natural OOD')
    ...
    if p == None:
        ...
        usedice = 'w_useless'
    known = np.loadtxt('{base_dir}/{in_dataset}/{method}/{name}/{usedice}/in_scores_p{p}_evr{evr}.txt'.format(base_dir=base_dir, in_dataset=in_dataset,usedice=usedice, method=method, p=p,evr=evr,name=name))
    ...
    for out_dataset in out_datasets:
        novel = np.loadtxt('{base_dir}/{in_dataset}/{method}/{name}/{usedice}/{out_dataset}/out_scores_p{p}_evr{evr}.txt'.format(base_dir=base_dir, in_dataset=in_dataset, method=method, usedice=usedice, name=name,  p=p,evr=evr,out_dataset=out_dataset))

        in_cond = (novel>threshold).astype(np.float32)
        a = np.mean(in_cond)
        print("{}---acc:  {:.2f}%".format(out_dataset,(1-a)*100))
